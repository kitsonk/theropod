...
class XMLHttpRequest extends XMLHttpRequestEventTarget {
  ...
  abort(): void {
    console.log(`xhr[${this.#uid}] abort()`);
    this.#terminate();
    if (
  ...
  dispatchEvent(evt: Event) {
    console.log(`xhr[${this.#uid}] dispatchEvent()`, { type: evt.type });
    switch (evt.type) {
  ...
  getAllResponseHeaders(): string | null {
    console.log(`xhr[${this.#uid}] getAllResponseHeaders()`);
    if (!this.#response) {
      return null;
    }
  ...
  getResponseHeader(name: string): string | null {
    console.log(`xhr[${this.#uid}] getResponseHeader()`, { name });
  ...
  overrideMimeType(mime: string): void {
    console.log(`xhr[${this.#uid}] overrideMimeType()`, { mime });
    if (this.#state === State.LOADING || this.#state === State.DONE) {
      throw new DOMException(
  ...
  send(body: BodyInit | null = null): void {
    console.log(`xhr[${this.#uid}] send()`, { body });
  ...
  setRequestHeader(name: string, value: string): void {
    console.log(`xhr[${this.#uid}] setRequestHeaders()`, { name, value });
    if (this.#state !== State.OPENED) {
      throw new DOMException("Invalid state", "InvalidStateError");
    }
    if (this.#sendFlag) {
